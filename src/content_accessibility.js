...
let getPathInfo = (_event) => {
  return Promise.resolve(_event)
    .then((event) => event.path || (event.composedPath && event.composedPath()))
    .catch(proxy.print.failure_get_browser_path);
};
...
let addDownloadButtonToFeed = (feedItem) => {
  ...
  const lastElem =
    feedItem.getElementsByClassName("feed-item-stats").length - 1;
...
let onDownloadClickFeed = (event) => {
  ...
  const feedUsername = feedItemContent
  ...
  downloadLinks.forEach((downloadLink) => {
    if (downloadLink.startsWith("img:")) {
      ...
      const name =
        feedUsername + "-" + downloadLink.split("/")[4].split("?")[0];
